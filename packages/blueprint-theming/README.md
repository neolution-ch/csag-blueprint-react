...
| `buildTenantCssVars`     | function | CSS overriding the accent custom properties. Uses `html:root` selectors so the overrides win over a stylesheet's `:root` rules in both colour schemes. Emits `--accent`, `--accent-muted`, `--accent-subtle` and `--glow` unless `varNames` renames them: these are the one part of this package that has to agree with a stylesheet it does not own. |
