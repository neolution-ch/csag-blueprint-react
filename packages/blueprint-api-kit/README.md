...
| `setErrorNotificationLabels` | function | Overrides that notification's five strings, or restores the English defaults with `null`. A module-level setter rather than a parameter, because the toast is raised from axios interceptors and query-cache handlers that do not sit inside the React tree holding your translations; call it whenever the resolved language changes. Non-string values keep their default, so feeding it a payload that has not loaded yet is safe. |
